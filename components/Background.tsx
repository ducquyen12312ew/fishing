...
function Bird({ top, size, duration, delay }: typeof BIRDS[number]) {
  return (
    <div style={{ position: 'absolute', top, left: 0, width: size, height: size * 0.45, animation: `birdFly ${duration}s linear ${delay}s infinite`, willChange: 'transform' }}>
      <div style={{ width: '100%', height: '100%', background: 'rgba(255,255,255,0.85)', animation: `birdWing ${duration / 8}s ease-in-out infinite`, clipPath: 'polygon(0% 60%, 25% 0%, 50% 50%, 75% 0%, 100% 60%)', willChange: 'clip-path' }} />
    </div>
  )
}
  ...
  return (
    <div style={{ position: 'absolute', top, left: '-70px', animation: `butterflyFly ${duration}s linear ${delay}s infinite`, willChange: 'transform' }}>
      <div style={{ position: 'relative', width: 24, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ position: 'absolute', left: 0, width: 12, height: 14, background: color, borderRadius: '60% 0 60% 40%', opacity: 0.85, animation: `butterflyWing 0.4s ease-in-out infinite`, transformOrigin: 'right center', willChange: 'transform' }} />
        <div style={{ position: 'absolute', right: 0, width: 12, height: 14, background: color, borderRadius: '0 60% 40% 60%', opacity: 0.85, animation: `butterflyWing 0.4s ease-in-out 0.2s infinite`, transformOrigin: 'left center', willChange: 'transform' }} />
...
export default function Background({ theme }: BackgroundProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {/* Fishing effects — only rendered when active */}
      {isFishing && (
        <>
          ...
          {LEAVES.map((l, i) => (
            <div
              ...
              style={{
                ...
                background: '#4a9960', borderRadius: '50% 0 50% 0',
          ...
          {jumpFish && (
            ...
            <img
              ...
              alt=""
              style={{
                position: 'absolute', bottom: '16%', left: `${jumpFish.x}%`,
                width: 32, imageRendering: 'pixelated',
          ...
          <div style={{ position: 'absolute', bottom: 0, left: '-5%', right: '-5%', height: '17%', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: '-30% 0 0 0', background: 'rgba(30,80,85,0.60)', animation: 'waveSway 7s ease-in-out infinite', willChange: 'transform' }} />
            <div style={{ position: 'absolute', inset: '-15% 0 0 0', background: 'rgba(85,165,170,0.40)', animation: 'waveSwayAlt 9s ease-in-out infinite', willChange: 'transform' }} />
            <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
              <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '40%', background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.18) 50%, transparent 100%)', animation: 'waterShimmer 4s linear infinite', willChange: 'transform' }} />
            </div>
            <div style={{ position: 'absolute', top: '6%', left: 0, right: 0, height: '2px', background: 'rgba(200,240,245,0.45)' }} />
            {BUBBLES.map((b, i) => (
              <div key={i} style={{ position: 'absolute', bottom: '70%', left: b.left, width: b.size, height: b.size, borderRadius: '50%', background: 'rgba(255,255,255,0.55)', border: '1px solid rgba(255,255,255,0.3)', animation: `bubbleRise ${b.duration}s ease-in ${b.delay}s infinite`, willChange: 'transform, opacity' }} />
            ))}
            <div style={{ position: 'absolute', top: '5%', left: '50%', width: 20, height: 20 }}>
              <div style={{ position: 'absolute', top: '50%', left: '50%', width: 20, height: 20, borderRadius: '50%', border: '2px solid rgba(180,230,235,0.6)', animation: 'rippleExpand 2s ease-out infinite', willChange: 'transform, opacity' }} />
              <div style={{ position: 'absolute', top: '50%', left: '50%', width: 20, height: 20, borderRadius: '50%', border: '2px solid rgba(180,230,235,0.4)', animation: 'rippleExpand 2s ease-out 1s infinite', willChange: 'transform, opacity' }} />
            </div>
          </div>
        </>
      )}

      {/* Football effects — only rendered when active */}
      {!isFishing && (
        <>
          {RAIN_DROPS.map((r, i) => (
            <div
          ...
          {LEAVES.slice(0, 3).map((l, i) => (
            <div key={i} style={{ position: 'absolute', top: '-20px', left: l.left, width: l.size * 0.7, height: l.size * 0.7, background: 'rgba(100,155,75,0.55)', borderRadius: '50% 0 50% 0', transform: `rotate(${l.rotate}deg)`, animation: `leafFall ${l.duration * 1.6}s ease-in ${l.delay}s infinite`, willChange: 'transform' }} />
          ))}
          <div style={{
